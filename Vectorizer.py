...
class CovidVectorizer(object):
    ...
    def __init__(self, predictor_df, predictor_vocab, target_vocab, args, max_predictor_length=0):
        """
        Args:
    ...
    def from_dataframe(cls, predictor_df, args):
        ...
        return cls(predictor_df, predictor_vocab, target_vocab, args, max_predictor_length)
'''
'''
class CovidVectorizer(object):
    """ The Vectorizer which coordinates the Vocabularies and puts them to use"""
...
class CovidVectorizer(object):
    ...
    def vectorize(self, predictor, classifier_class):
        ...
        for token in predictor.split(" "):
            one_hot_matrix[self.predictor_vocab.lookup_token(token)] = self.predictor_vocab.lookup_token(token)
